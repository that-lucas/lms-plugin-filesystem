...
const runPush = (cwd, args) => new Promise((resolve, reject) => {
  const child = spawn("lms", ["push", ...args], {
    cwd,
    stdio: "inherit",
  })
  ...
  child.on("exit", (code, signal) => {
    if (signal) {
      reject(new Error(`lms push exited from signal ${signal}`))
      return
    }
    resolve(code ?? 1)
  })
...
const syncManifestBack = async (tempRoot) => {
  const tempManifest = path.join(tempRoot, "manifest.json")
  const projectManifest = path.join(projectRoot, "manifest.json")
  await fs.copyFile(tempManifest, projectManifest)
}

async function main() {
  const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "lms-plugin-push-"))
